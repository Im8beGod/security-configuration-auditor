...
def test_migration_history_extends_parentless_baseline() -> None:
    ...
    assert script.get_heads() == [STEP_7_1_REVISION]
    assert len(revisions) == 7
    assert revisions[0].revision == STEP_7_1_REVISION
    assert revisions[0].down_revision == STEP_6A_REVISION
    assert revisions[1].revision == STEP_6A_REVISION
    assert revisions[1].down_revision == STEP_5C_REVISION
    assert revisions[2].revision == STEP_5C_REVISION
    assert revisions[2].down_revision == STEP_3_9_REVISION
    assert revisions[3].revision == STEP_3_9_REVISION
    assert revisions[3].down_revision == STEP_3_6_REVISION
    assert revisions[4].revision == STEP_3_6_REVISION
    assert revisions[4].down_revision == CURRENT_REVISION
    assert revisions[5].revision == CURRENT_REVISION
