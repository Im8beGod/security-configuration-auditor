...
def _definition(field: str, path: list[str], source: str, output: str, capture: str | None = None, attribute: str | None = None, *, source_type: str | None = None, unit_conversion: str = "none", qualifiers: list[list[str]] | None = None) -> dict:
    return {
        "profile_applicability": {"profile_ids": ["juniper.junos.18"], "profile_version_ids": [PROFILE]},
        "structural_match": {"operation": "xml_path", "command": "xml", "xml_path": {"path": [{"local_name": item, "occurrence": "any" if item == "server" else "exact"} for item in path], "source": source, "capture": capture, "attribute": attribute, "value_type": source_type or output, "start_mode": "document_root"}},
        ...
        "unit_conversion": {"operation": unit_conversion}, "scope_resolution": {"strategy": "device", "xml_unsupported_qualifier_paths": qualifiers or []},
...
MAPPINGS = B4_MAPPINGS[:2] + (
    DeclarativeMapping(
        UUID("b3010000-0000-5000-8000-000000000003"),
        UUID("b3020000-0000-5000-8000-000000000103"),
        "time.ntp.server", NodeMatcher(command="xml"), "training_dsl", "device",
        frozenset({TypedValueType.STRING}),
        training_definition=_definition(
            "time.ntp.server", ["configuration", "system", "ntp", "server", "name"],
            "text", "string", "server", qualifiers=[["routing-instance"]],
        ),
    ),
    DeclarativeMapping(
        UUID("b3010000-0000-5000-8000-000000000004"),
        UUID("b3020000-0000-5000-8000-000000000104"),
        "management.session.idle_timeout", NodeMatcher(command="xml"), "training_dsl", "device",
        frozenset({TypedValueType.DURATION}),
        training_definition=_definition(
            "management.session.idle_timeout", ["configuration", "system", "login", "idle-timeout"],
            "text", "duration", "minutes", source_type="integer", unit_conversion="minutes_to_seconds",
        ),
    ),
    DeclarativeMapping(
        UUID("b3010000-0000-5000-8000-000000000005"),
        UUID("b3020000-0000-5000-8000-000000000105"),
        "time.ntp.configured", NodeMatcher(command="xml"), "training_dsl", "device",
        frozenset({TypedValueType.BOOLEAN}),
        training_definition=_definition(
            "time.ntp.configured", ["configuration", "system", "ntp", "server"],
            "presence", "boolean", qualifiers=[["routing-instance"]],
        ),
    ),
)
