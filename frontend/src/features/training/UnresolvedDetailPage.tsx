...
function MappingWorkflow({ block, mappingId, fields }: { block: UnresolvedBlock; mappingId: string; fields: CanonicalField[] }) {
  ...
  const validate = useMutation({ mutationFn: () => validateMapping(mappingId, mapping.data?.structural_match.operation === 'xml_path' ? block.evidence_refs.find((ref) => typeof ref.artifact_id === 'string')?.artifact_id as string | undefined : undefined), onSuccess: refresh })
