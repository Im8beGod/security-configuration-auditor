...
export async function validateMapping(id: string, evidenceArtifactId?: string, negativeEvidenceArtifactId?: string): Promise<ValidationRequest> { return await apiRequest(`/training/mappings/${id}/validate`, { method: 'POST', body: json({ ...(evidenceArtifactId ? { evidence_artifact_id: evidenceArtifactId } : {}), ...(negativeEvidenceArtifactId ? { negative_evidence_artifact_id: negativeEvidenceArtifactId } : {}) }) }) as ValidationRequest }
