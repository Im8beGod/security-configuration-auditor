...
P=[('cisco.ios_xe.17@1.0.0','management.telnet.disabled','cisco.ssh-only-vty',{'vty_range':'0 4'}),('fortinet.fortios.7@1.0.0','management.telnet.disabled','fortios.secure-interface-access',{'interface':'port1','protocols':'https ssh'}),('juniper.junos.18@1.0.0','management.telnet.disabled','junos.ssh-only-service',{})]
def test_b10_persisted_previews():
 e=create_database_engine(get_settings());f=create_session_factory(e)
 with f.begin() as d:
  assert d.scalar(text('select version_num from alembic_version'))=='20260923_0025';assert d.scalar(text("select count(*) from remediation_procedures"))==11
  ...
  with pytest.raises(DatabaseError,match='immutable'):
   with d.begin_nested(): d.execute(text("update remediation_procedures set title='x' where procedure_id=:i").bindparams(i=q.procedure_id))
 e.dispose()
