...
def test_b9_alignment_is_immutable_pinned_and_multivendor():
    ...
    try:
      ...
      with factory.begin() as db:
        assert db.scalar(text("SELECT version_num FROM alembic_version")) == "20260922_0024"
        ...
        for n,profile in enumerate(PROFILES,1):
          ...
          expected={PROFILES[0]:(8,0),PROFILES[1]:(7,1),PROFILES[2]:(5,3)}[profile]
          assert pin.assessment_pack_version_id==pack.assessment_pack_version_id and coverage["automatic_verdicts"]=={"pass":expected[0],"fail":0,"unknown":0} and coverage["not_applicable"]==expected[1] and coverage["manual"]==2 and results["iso27001-a.5.14-remote-ssh-enabled"].verdict=="pass"
          if n==1:
