...
const emptyDefinition = (profileVersionId: string): MappingDefinition => ({
  ...
  structural_match: profileVersionId.startsWith('generic.json') ? { operation: 'json_path', command: 'json', json_path: { path: [], source: 'value', capture: 'value', value_type: 'string' } } : { operation: 'xml_path', command: 'xml', xml_path: { path: [], source: 'text', capture: 'value', value_type: 'string', start_mode: 'document_root' } },
...
export function MappingEditorPage() {
  ...
  const [negativeArtifactId, setNegativeArtifactId] = useState('')
  ...
  const validate = useMutation({ mutationFn: () => validateMapping(mappingId as string, artifactId || undefined, negativeArtifactId || undefined), onSuccess: () => void queryClient.invalidateQueries({ queryKey: ['training-mapping', mappingId] }) })
  ...
  const structured = ['xml_path', 'json_path'].includes(String(selected?.structural_match.operation))
  ...
  return <section className="page-stack training-detail"><Link className="back-link" to="/training">← Review Center</Link><header className="page-heading"><div><span className="eyebrow">Bounded declarative controls</span><h1>{selected ? selected.title : 'Create Mapping'}</h1><p>Server validation remains authoritative. No code, regex, shell, or executable expressions are accepted.</p></div>{selected && <StatusBadge value={selected.status} />}</header><QueryState pending={Boolean(mappingId) && existing.isPending} error={existing.error}><>{!selected && <section className="panel form-grid"><label className="field">Mapping key<input value={mappingKey} onChange={(event) => setMappingKey(event.target.value)} /></label><label className="field">Compatible profile<select value={profileVersionId} onChange={(event) => { setProfileVersionId(event.target.value); setDefinitionText(JSON.stringify(emptyDefinition(event.target.value), null, 2)) }}>{profileOptions.map((profile) => <option key={profile.profile_version_id} value={profile.profile_version_id}>{profile.profile_version_id}</option>)}</select></label></section>}<section className="panel form-grid"><label className="field">Title<input value={title} onChange={(event) => setTitle(event.target.value)} /></label><label className="field span-two">Description<textarea rows={3} value={description} onChange={(event) => setDescription(event.target.value)} /></label><label className="field span-two">Complete MappingDefinition JSON<small>Lossless bounded editor for exact XML/JSON paths, extraction, and scope.</small><textarea rows={24} value={definitionText} onChange={(event) => setDefinitionText(event.target.value)} /></label></section>{save.isError && <p className="error-message" role="alert">{(save.error as Error).message}</p>}<div className="button-row"><button className="button-primary" disabled={save.isPending} onClick={() => save.mutate()}>{save.isPending ? 'Saving...' : selected ? 'Save draft' : 'Create draft Mapping'}</button></div>{selected && <><section className="panel"><div className="section-title"><div><span className="eyebrow">Development evidence</span><h2>Run executable validation</h2></div></div><label className="field">Positive evidence artifact<select value={artifactId} onChange={(event) => setArtifactId(event.target.value)}><option value="">{structured ? 'Select matching evidence' : 'Use examples only'}</option>{readyArtifacts.map((artifact) => <option key={artifact.artifact_id} value={artifact.artifact_id}>{artifact.original_filename} · {artifact.evidence_type}</option>)}</select></label>{structured && <label className="field">Negative evidence artifact<select value={negativeArtifactId} onChange={(event) => setNegativeArtifactId(event.target.value)}><option value="">Select non-matching evidence</option>{readyArtifacts.filter((artifact) => artifact.artifact_id !== artifactId).map((artifact) => <option key={artifact.artifact_id} value={artifact.artifact_id}>{artifact.original_filename} · {artifact.evidence_type}</option>)}</select></label>}<div className="button-row">{['draft', 'testing'].includes(selected.status) && <button className="button-primary" disabled={validate.isPending || selected.status === 'testing' || (structured && (!artifactId || !negativeArtifactId))} onClick={() => validate.mutate()}>{validate.isPending ? 'Queueing...' : 'Validate selected evidence'}</button>}{selected.status === 'testing' && <span className="quiet-state">Validation is running. This page refreshes automatically.</span>}{selected.status === 'testing' && selected.validation_results.passed === true && <button className="button-primary" onClick={() => approve.mutate()}>Approve mapping</button>}{selected.status === 'approved' && <button className="button-primary" onClick={() => publish.mutate()}>Publish immutable Mapping</button>}</div></section><SemanticResult mapping={selected} /></>}</></QueryState></section>
}
