...
def test_migration_history_extends_parentless_baseline() -> None:
    ...
    assert script.get_heads() == [B10_REMEDIATION_REVISION]
    assert len(revisions) == 18
    expected = [B10_REMEDIATION_REVISION, ISO_ALIGNMENT_REVISION, CIS_PACK_REVISION, DISA_PACK_REVISION, NIST_PACK_REVISION, PROFILE_MANIFEST_REVISION, ASSESSMENT_PACK_REVISION, STEP_11_1_REVISION, STEP_10A_REVISION, STEP_9_2_REVISION, STEP_9_1_REVISION, STEP_7_1_REVISION, STEP_6A_REVISION, STEP_5C_REVISION, STEP_3_9_REVISION, STEP_3_6_REVISION, CURRENT_REVISION, BASELINE_REVISION]
    assert [item.revision for item in revisions] == expected
    assert revisions[0].down_revision == ISO_ALIGNMENT_REVISION
